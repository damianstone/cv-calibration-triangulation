def refine_and_deduplicate_lines(
    raw_lines, white_mask, refine_dist=6, angle_thresh_degs=0.75, dist_thresh=1.5
):
    """
    ...
    for (x1, y1, x2, y2) in raw_lines:
        ...
        pts = []
        minx, maxx = max(0, min(x1, x2) - refine_dist), min(
            width - 1, max(x1, x2) + refine_dist
        )
        miny, maxy = max(0, min(y1, y2) - refine_dist), min(
            height - 1, max(y1, y2) + refine_dist
        )
