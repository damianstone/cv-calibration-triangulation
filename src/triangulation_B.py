...
def get_average_error_cm(detections_csv):
    ...
    if len(valid_detections) == 0:
        return 0, 0
    a1_total_cm_error = valid_detections['B1_error_cm'].sum()
    average_cm_error_a1 = a1_total_cm_error / len(valid_detections)
    a2_total_cm_error = valid_detections['B2_error_cm'].sum()
...
def preprocess_detections(detections_csv, threshold=0.3):
    ...
    detections = detections[detections['B1_confidence'] > threshold]
    detections = detections[detections['B2_confidence'] > threshold]
...
def triangulate(
    ...
    for index, row in detections_csv.iterrows():
        ...
        cam1_2d = (row['B1_x'] * scale_factor, row['B1_y'] * scale_factor)
        cam2_2d = (row['B2_x'] * scale_factor, row['B2_y'] * scale_factor)
        ...
        if e1_px > pixel_threshold or e2_px > pixel_threshold:
            ...
            print(f"ORIGINAL B1: {tuple(map(lambda x: round(float(x), 2), cam1_2d))}")
            print(
                f"REPROJECTION B1: {tuple(map(lambda x: round(float(x), 2), cam1_reprojected_point))}")
            print(f"ORIGINAL B2: {tuple(map(lambda x: round(float(x), 2), cam2_2d))}")
            print(
                f"REPROJECTION B2: {tuple(map(lambda x: round(float(x), 2), cam2_reprojected_point))}")
            print(f"ERROR B1 (cm): {round(float(e1_cm), 2)}")
            print(f"ERROR B2 (cm): {round(float(e2_cm), 2)}")
        ...
        detections_csv.at[index, 'B1_reprojected_point'] = cam1_reprojected_str
        detections_csv.at[index, 'B1_error_cm'] = e1_cm
        detections_csv.at[index, 'B2_reprojected_point'] = cam2_reprojected_str
        detections_csv.at[index, 'B2_error_cm'] = e2_cm
    ...
    B1_threshold_cm, B2_threshold_cm = get_average_cm_threshold(
    ...
    return {
        ...
        "B1_average_cm_error": round(float(b1_average_cm_error), 2),
        "B2_average_cm_error": round(float(b2_average_cm_error), 2),
        "B1_threshold_cm": round(float(B1_threshold_cm), 2),
        "B2_threshold_cm": round(float(B2_threshold_cm), 2)
    }
...
if __name__ == "__main__":
    ...
    Fx_cam1 = intrinsic_params['CAM_3']['fx']
    K_cam1_undistort = np.array(intrinsic_params['CAM_3']['K_undistort'])
    D_cam1 = np.array(intrinsic_params['CAM_3']['D'])

    Fx_cam2 = intrinsic_params['CAM_4']['fx']
    K_cam2_undistort = np.array(intrinsic_params['CAM_4']['K_undistort'])
    D_cam2 = np.array(intrinsic_params['CAM_4']['D'])

    stereo_rotation_matrix = np.array(extrinsics['STEREO_B']['rotation_matrix'])
    stereo_translation_vector = np.array(extrinsics['STEREO_B']['translation_vector'])
    ...
    with open(f"{root}/output/2_triangulation_B.json", 'w') as f:
