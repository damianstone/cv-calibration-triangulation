...
def plot_2d_reprojected_points(triangulation_results, stereo='A', camera_names=['Camera 1', 'Camera 2']):
    ...
    # Extract points
    points_2d_1 = np.array([eval(p) for p in triangulation_results[f'{stereo}1_reprojected_point']])
    points_2d_2 = np.array([eval(p) for p in triangulation_results[f'{stereo}2_reprojected_point']])
...
def plot_valid_vs_original(triangulation_results, stereo='A', camera_names=['Camera 1', 'Camera 2']):
    ...
    scale_factor=2
    # Extract points
    original_2d_1 = np.array([[row[f'{stereo}1_x'] * scale_factor, row[f'{stereo}1_y'] * scale_factor] for _, row in triangulation_results.iterrows()])
    points_2d_1 = np.array([eval(p) for p in triangulation_results[f'{stereo}1_reprojected_point']])
    original_2d_2 = np.array([[row[f'{stereo}2_x'] * scale_factor, row[f'{stereo}2_y'] * scale_factor] for _, row in triangulation_results.iterrows()])
    points_2d_2 = np.array([eval(p) for p in triangulation_results[f'{stereo}2_reprojected_point']])
    anomaly_mask = triangulation_results['anomaly_detected']
    ...
    ax1.scatter(points_2d_1[~anomaly_mask, 0], points_2d_1[~anomaly_mask, 1], c='green', alpha=0.6, label='Reprojected')
    ...
    ax2.scatter(points_2d_2[~anomaly_mask, 0], points_2d_2[~anomaly_mask, 1], c='green', alpha=0.6, label='Reprojected')
